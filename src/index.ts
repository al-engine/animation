import { DeltaTime, log, LogLevel, Asset, SpriteMap, Sprite } from '@al-engine/core';
...
export class AnimationData {
  ...
  getAnimation(name: string) {
    ...
    if (!animation) {
      log(
        `Animation with name ${name} doesn't not exist`,
        LogLevel.error,
        'rendering'
      );
    }
...
export class SpriteAnimation {
  ...
  private continue(
    ...
    delta: DeltaTime,
    timer: number,
    resolveIndexOverflow: (index: number, length: number) => number
  ) {
    const animation = this.animationData.getAnimation(name);
    if (!animation) {
      return;
